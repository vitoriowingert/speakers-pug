class SpeakersManager {
  ...
  createSpeakerCard(speaker) {
    return `
      <li class="speakers__grid__card">
        ...
        >
          <div class="speakers__grid__card__image">
            <img src="${speaker.image}" alt="" aria-hidden="true">
          </div>
        </button>
        <div class="speakers__grid__card__info">
          <h3 class="speakers__grid__card__info__name">${speaker.name}</h3>
          <p class="speakers__grid__card__info__role">${speaker.role}</p>
          <p class="speakers__grid__card__info__company">${speaker.company}</p>
        </div>
      </li>
    `;
  }
  ...
  renderSpeakerPopup(id) {
    ...
    return `
      <div class="speakers__details speaker__${speaker.id}">
        <div class="speakers__details__overlay"></div>
        <div class="speakers__details__content">
          <div class="speakers__details__content__header">
            <h3 class="speakers__details__content__header__name">${speaker.name}</h3>
            <button class="speakers__details__content__header__close" onclick="speakersManager.handleClosePopup(${speaker.id})">
          ...
          <hr />
          <div class="speakers__details__content__text">
            <div class="speakers__details__content__text__inner">${speaker.details}</div>
          </div>
        </div>
      </div>
    `;
  ...
  handleClosePopup(id) {
    const popup = document.querySelector(`.speakers__details.speaker__${id}`);
  ...
  handleEscKeyPress(id) {
    document.addEventListener("keydown", (event) => {
      if (event.key === "Escape") {
        const activePopup = document.querySelector(".speakers__details.active");
        if (activePopup) {
